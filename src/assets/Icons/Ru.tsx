
export const Ru = () => (
  <svg height="24" width="24" fill="none" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    <rect height="16.6429" width="23.5" fill="white" rx="1.75" stroke="#F5F5F5" strokeWidth="0.5" x="0.25" y="3.67999"/>
    <mask height="18" id="mask0_2296_1552" style={{"maskType":"luminance"}} width="24" x="0" y="3" maskUnits="userSpaceOnUse">
      <rect height="16.6429" width="23.5" fill="white" rx="1.75" stroke="white" strokeWidth="0.5" x="0.25" y="3.67999"/>
    </mask>
    <g mask="url(#mask0_2296_1552)">
      <path d="M0 14.8586H24V9.14435H0V14.8586Z" fill="#0C47B7" fillRule="evenodd"/>
      <path d="M0 20.573H24V14.8587H0V20.573Z" fill="#E53B35" fillRule="evenodd"/>
    </g>
  </svg>
  );